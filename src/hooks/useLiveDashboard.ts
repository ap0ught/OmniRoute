...
export function useLiveDashboard({
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/ws?handshake=1")
      ...
      .then((body) => {
        ...
        const port = body?.live?.port && !isNaN(Number(body?.live?.port)) && Number(body?.live?.port) > 0 && Number(body?.live?.port) < 65536 ? Number(body?.live?.port) : null;
  ...
  // Resolve WS URL: use explicit if provided, otherwise derive from handshake info

let effectiveWsUrl = wsUrl;

if (!effectiveWsUrl) {

  // Derive path from handshake info

  const handshakePathVal = handshakePath !== BUILD_TIME_WS_PATH ? handshakePath : null;

  const wsPath = deriveLiveWsPath(handshakePathVal ? undefined : process.env.NEXT_PUBLIC_LIVE_WS_PUBLIC_URL);

  effectiveWsUrl = `ws://localhost:20132${wsPath}`;

}

if (!effectiveWsUrl && handshakeUrl) {

  effectiveWsUrl = handshakeUrl;
